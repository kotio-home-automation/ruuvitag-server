...
const formatFloatValue = (value) => parseFloat(value.toFixed(2))
...
const parseFormat3RawTagData = (rawData, uuid) => {
  ...
  const tagData = {
    temperature: formatFloatValue(temperature),
    pressure: formatFloatValue(pressure),
    humidity: formatFloatValue(humidity)
  }
...
const parseFormat5RawTagData = (rawData, uuid) => {
  ...
  tagData.temperature = formatFloatValue(temperature / 200.00)

  const humidity = ((((rawData[5] & 0xff) << 8) | (rawData[6] & 0xff)) / 400.0)
  tagData.humidity = formatFloatValue(humidity)
  const pressure = (((rawData[7] & 0xff) << 8) | (rawData[8] & 0xff)) + 50000;
  tagData.pressure = (pressure > 1100) ? null : formatFloatValue(pressure)
